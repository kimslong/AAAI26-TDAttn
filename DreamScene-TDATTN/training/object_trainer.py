...
class ObjectTrainer:
    def __init__(self, cfg, renderer=None, id=None, gs_obj=None, device=None) -> None:
        ...
        exp_root = Path(f"experiments/")
        exp_path = Path(cfg.objectParams["id"])
        self.ckpt_path = make_path(exp_root /exp_path /"checkpoints")
        self.train_renders_path = make_path(exp_root /exp_path/ "vis" / "train")
        self.eval_renders_path = make_path(exp_root/exp_path / "vis" / "eval")
        self.train_steps = 1
    ...
    def train_step(self) -> None:
        ...
        for _ in range(self.train_steps):
            ...
            self.renderer.c_batchsize = C_batch_size
            ...
            for i in range(C_batch_size):
                ...
                render_attn = out["render_attn"][0, :, :]
                depth, alpha = out["depth"], out["alpha"]
                scales.append(out["scales"])
                images.append(image)
                depths.append(depth)
                alphas.append(alpha)
                render_attns.append(render_attn)
                azimuths.append(viewpoint_cam.delta_azimuth)
                viewpoint_cams.append(viewpoint_cam)

            images = torch.stack(images, dim=0)
            depths = torch.stack(depths, dim=0)
            alphas = torch.stack(alphas, dim=0)
            self.renderer.azimuths = azimuths
            self.renderer.viewpoint_cams=viewpoint_cams
            # Loss
            _aslatent = False
            ...
            loss = self.guidance.train_step(
                ...
                vds=vds,
                obj_id=gs_obj.id,
                render_attns0=render_attns,
                renderer=self.renderer,
                tokenizer=self.guidance.tokenizer
            )
    ...
    def train(self) -> None:
        ...
        self.prepare_train()
        self.renderer.object_gaussians_dict[self.renderer.cfg.objectParams["id"]].model.GSweights = torch.zeros_like(self.gs_obj.model.get_opacity)
        self.renderer.object_gaussians_dict[self.renderer.cfg.objectParams["id"]].model.GSweights_cnt = torch.zeros_like(self.gs_obj.model.get_opacity,dtype=torch.int32)
        import TDATTN_SGT
        import numpy as np
        from TDATTN_SGT import tdattn_sgt
        import csv
        self.renderer.c_batchsize = self.guidance_opt.C_batch_size
        self.renderer.no_key = self.guidance_opt.no_key
        data = []
        with open(self.renderer.description_file_path, "r") as f:
        ...
        self.renderer.SGT = data

        self.renderer.prompts = [self.gs_obj.text["text"]]
        self.renderer.zero_prompt_equalizer = TDATTN_SGT.get_equalizer(self.renderer.prompts[0], (self.renderer.selected_token), (0,), tokenizer=self.renderer.tokenizer).cuda()
        self.renderer.equalizers.append(self.renderer.zero_prompt_equalizer)
        self.renderer.equalizers.append(1 - self.renderer.zero_prompt_equalizer)
        self.renderer.tokenizer = self.guidance.tokenizer
