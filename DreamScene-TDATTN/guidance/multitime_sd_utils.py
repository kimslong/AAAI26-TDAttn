...
class StableDiffusion(nn.Module):
    ...
    def train_step(
        ...
        with torch.no_grad():
            ...
            # cfg=guidance_opt.denoise_guidance_scale,
            ...
            for next_ind_t in rand_list:
                ...
                standard_sd = True
                controller = TDATTN_SGT.AttentionReweight(num_steps=timestep_model_input[0],
                                                          cross_replace_steps=[0.0, 0.0],
                                                          equalizers=renderer.equalizers,
                                                          adjusting_direction=['View', 'View'],
                                                          category_vectors=renderer.category_vectors,
                                                          head_index=renderer.head_index,
                                                          standard_sd=standard_sd, skip_call=True,
                                                          tokenizer=tokenizer,
                                                          SGT=renderer.SGT)
                renderer.register_attention_control(self, controller)
            ...
            if iteration<5000:
                standard_sd = True
                skip_call = False
                if np.mean(np.abs(renderer.azimuths)) > 150 and random.random() < 0.1:
                    standard_sd = False
                    skip_call = False
                controller = TDATTN_SGT.AttentionReweight(num_steps=timestep_model_input[0],
                                                          cross_replace_steps=renderer.reweight_steps,
                                                          equalizers=renderer.equalizers,
                                                          adjusting_direction=['View', 'View'],
                                                          category_vectors=renderer.category_vectors,
                                                          head_index=renderer.head_index,
                                                          standard_sd=standard_sd, skip_call=skip_call,
                                                          tokenizer=tokenizer, batch_size=renderer.c_batchsize,
                                                          SGT=renderer.SGT)
                renderer.register_attention_control(self, controller)
        ...
        id = renderer.cfg.objectParams["id"]
        show_flag = 0
        if controller is not None and not skip_call and iteration >100:
            if iteration % 20 == 0:
                save_2d_attn = 1
            else:
                save_2d_attn = 0
            # if iteration % 10 == 0:
            save_path_iter_attn = os.path.join(save_folder, "iter_{}_step_{}_attn".format(iteration, timestep_model_input[0].item()))
            image_key32 = renderer.show_cross_attention(tokenizer, save_path_iter_attn,
                                                      attention_store, res=32,
                                                      from_where=("up", "down"), save_2d_attn=save_2d_attn,
                                                      no_key=renderer.no_key)
            # del controller, delta_noise_preds, latent_model_input, noise_pred_text, target, text_embeddings, unet_output
            # torch.cuda.empty_cache()
            # torch.cuda.ipc_collect()
            # tokens = self.tokenizer.encode(self.prompts[0])and iteration >100
            if iteration < 1000 :
                show_flag = 1
                ...
                if renderer.c_batchsize == 1:
                    gray = image_key32[:, :, 0]
                    attn_2d = torch.tensor(gray, dtype=torch.float32).unsqueeze(0).cuda()
                    renderer.update_mask(renderer.viewpoint_cams[0], attn_2d)
                else:
                    for i in range(renderer.c_batchsize):
                        gray = image_key32[i]
                        attn_2d = torch.tensor(gray, dtype=torch.float32).unsqueeze(0).cuda()
                        renderer.update_mask(renderer.viewpoint_cams[i], attn_2d)
    ...
    def addnoise_with_cfg(
        ...
        for next_ind_t in rand_list:
            ...
            if renderer!=None:
                standard_sd = True
                controller = TDATTN_SGT.AttentionReweight(num_steps=timestep_model_input[0], cross_replace_steps=[0.0, 0.0],
                                                          equalizers=renderer.equalizers,
                                                          adjusting_direction=['View', 'View'],
                                                          category_vectors=renderer.category_vectors,
                                                          head_index=renderer.head_index,
        ...
        if renderer != None:
            ...
            controller = TDATTN_SGT.AttentionReweight(num_steps=timestep_model_input[0], cross_replace_steps=[0.0, 0.0],
                                                      ...
                                                      category_vectors=renderer.category_vectors,
                                                      head_index=renderer.head_index,
                                                      standard_sd=standard_sd, skip_call=skip_call, tokenizer=tokenizer, batch_size=renderer.c_batchsize,
                                                      SGT=renderer.SGT)
