...
def render(viewpoint_camera, pc, pipe, bg_color: torch.Tensor, scaling_modifier=1.0, black_video=False,
    ...
    # opacity_attn = torch.ones_like(opacity)
    # min_val = pc.GSweights.min()
    # max_val = pc.GSweights.max()
    # normalized_GS = (pc.GSweights - min_val) / (max_val - min_val)
    # opacity_attn = 1 - normalized_GS
    ...
    if colors_precomp is None:
        if pipe.convert_SHs_python:
            ...
            if pc.GSweights != None:
                if pc.GSweights != None:
                    shs_attn = pc.GSweights.unsqueeze(-1).repeat(1, 1, 3)  # (N, 1) -> (N, 1, 3)
        else:
            shs = pc.get_features
            if pc.GSweights != None:
                shs_attn = pc.GSweights.unsqueeze(-1).repeat(1, 1, 3)  # (N, 1) -> (N, 1, 3)
            # if pc.smoothedGS!=None:
            #     shs_smoothedGS = pc.smoothedGS.unsqueeze(-1).repeat(1, 1, 3)
    ...
    if pc.GSweights != None:
        rendered_attn, _, _ = rasterizer(
            ...
            cov3D_precomp=cov3D_precomp)
    # if pc.smoothedGS!=None:
    #     rendered_smoothedGS, _, _ = rasterizer(
    #         means3D=means3D,
    #         means2D=means2D,
    #         shs=shs_smoothedGS,
    #         colors_precomp=colors_precomp,
