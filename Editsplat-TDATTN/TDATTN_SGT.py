...
class tdattn_sgt(GaussianModel):
    ...
    def show_cross_attention(self, tokenizer, save_path_iter_attn, attention_store, res: int, from_where,
                             select: int = 0, save_2d_attn=1, no_key: int = -1000,prompt=None,step=0):
        ...
        if 1:
            if save_2d_attn:
                for i in range(len(tokens)):
                    ...
                    image = np.array(pil_image.resize((256, 256)))
                    if i == no_key:
                        image_key = np.array(pil_image.resize((512, 512)))
                    images.append(image)
                concatenated_image = np.concatenate(images, axis=1)
                save_path_with_res = f"{save_path_iter_attn}/{step}_{res}x{res}.jpg"
                save_path_with_res_no_key = f"{save_path_iter_attn}/{step}_sbj_{res}x{res}.jpg"
                pil_image = Image.fromarray(concatenated_image)
                pil_image.save(save_path_with_res,quality = 95)
                pil_image = Image.fromarray(image_key)
                pil_image.save(save_path_with_res_no_key,quality = 95)
...
class AttentionStore(AttentionControl):
    ...
    def forward(self, attn, is_cross: bool, place_in_unet: str, layer):
        ...
        if attn.shape[1] == 32 ** 2 and is_cross:
            attn_load1 = attn.reshape(1, 8, attn.shape[1], 77)
            attn_load2 = attn_load1.view(1, 8, 32, 32, 77)
...
class AttentionControlEdit(AttentionStore, abc.ABC):
    ...
    def __init__(self, prompts,num_steps: int,
        ...
        self.cross_replace_alpha = get_time_words_attention_alpha(prompts,num_steps, cross_replace_steps, tokenizer).cuda()
...
class AttentionReweight(AttentionControlEdit):
    ...
    def replace_cross_attention(self, attn, place_in_unet=None, layer=None, head=None, SGT=None):
        ...
        desired_concept_rescaler2 = self.category_vectors[
        ...
        undesired_concept_rescaler = self.category_vectors[
            self.SGT.index(self.adjusting_direction[2]), np.where(self.head_index == head_pos)[0][0]]
        HAB = 2*desired_concept_rescaler-undesired_concept_rescaler
        rescale_factor = (self.equalizers[0][:, None, :] + self.equalizers[1][:, None, :] * (HAB))  # concept adjusting

        attn = attn * rescale_factor

        return attn

    def __init__(self, prompts,num_steps=20, cross_replace_steps=None, equalizers=None, adjusting_direction=None,
                 category_vectors=None, head_index=None, standard_sd=None, skip_call=False, batch_size=1,
                 tokenizer=None, SGT=None):
        super(AttentionReweight, self).__init__(prompts,num_steps, cross_replace_steps, skip_call, batch_size, tokenizer)
